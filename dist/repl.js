  ...
  "node_modules/which/which.js"(exports2, module2) {
    ...
    var which = (cmd2, opt, cb) => {
      ...
      const step = (i) => new Promise((resolve, reject) => {
        ...
        const pCmd = path.join(pathPart, cmd2);
...
var errorHandler = (error) => {
  if ((error == null ? void 0 : error.cause) && JSON.stringify(error.cause).includes("ECONNREFUSED")) {
...
function commandLoop() {
  readline.question("\n> ", async (input) => {
    var _a;
    const args2 = input.trim().split(/\s+/);
    const command = (_a = args2[0]) == null ? void 0 : _a.toLowerCase();
    try {
      switch (command) {
        case "list-tools":
          await listTools();
        ...
        case "call-tool":
          if (args2.length < 2) {
            ...
            const toolName = args2[1];
            let toolArgs = {};
            if (args2.length > 2) {
              try {
                toolArgs = JSON.parse(args2.slice(2).join(" "));
              } catch {
        ...
        case "get-prompt":
          if (args2.length < 2) {
            ...
            const promptName = args2[1];
            let promptArgs = {};
            if (args2.length > 2) {
              try {
                promptArgs = JSON.parse(args2.slice(2).join(" "));
              } catch {
                console.log("Invalid JSON arguments. Using empty args.");
              }
            }
...
Resource list changed notification received!`);
    ...
    sessionId = transport2.sessionId;
...
async function callTool(name, args2) {
  ...
  try {
    ...
    console.log(`Calling tool '${name}' with args:`, args2);
    ...
    (_a = result == null ? void 0 : result.content) == null ? void 0 : _a.forEach((item) => {
      if (item.type === "text") {
...
async function getPrompt(name, args2) {
  if (!client) {
    console.log("Not connected to server.");
    return;
  }
