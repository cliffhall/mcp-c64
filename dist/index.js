...
async function assembleProgram({ command, sourcePath, args }) {
  const commandArgs = args || [];
  const child = (0, import_child_process.spawn)(command, [sourcePath, ...commandArgs]);
  let stdoutData = "";
  let stderrData = "";
  let exitCode;
  let response = null;
...
var NoArgSchema = external_exports.object({});
var AssembleProgramSchema = external_exports.object({
  command: external_exports.string().nonempty("Command is required"),
  sourcePath: external_exports.string().nonempty("Source path is required"),
